...
async function registerAndConfirm(
  ...
  if (!match) throw new Error("No confrim URL found in email");
...
describe("update password flow integration", () => {
  it("updates password with valid token from email link", async () => {
    const email = randomEmail();
    const oldPassword = "TestPassword123";
    const newPassword = "NewPassword123";
    const nickname = `up${Date.now()}`;

    await registerAndConfirm(email, oldPassword, nickname);

    await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${APP_URL}/auth/update-password`,
    });
    const recoveryUrl = await getRecoveryUrlFor(email);
    await fetch(recoveryUrl, { redirect: "manual" });

    const { data: signInData, error: signInError } =
      await supabase.auth.signInWithPassword({ email, password: oldPassword });
    expect(signInError).toBeNull();

    const authedClient = createClient<Database>(SUPABASE_URL, SUPABASE_KEY);
    ...
    expect(oldPassError).not.toBeNull();

    const { data: newSignIn, error: newPassError } =
      await supabase.auth.signInWithPassword({ email, password: newPassword });
    expect(newPassError).toBeNull();
    expect(newSignIn.session).not.toBeNull();
  });
});
